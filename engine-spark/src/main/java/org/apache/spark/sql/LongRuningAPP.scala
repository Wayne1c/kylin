...
object LongRuningAPP extends Logging {
  @volatile
  ...
  def getActiveJobs(): Int = {
    LongRuningAPP.getSparkSession.sparkContext.jobProgressListener.activeJobs.size
  }

  def getFailedJobs(): Int = {
    LongRuningAPP.getSparkSession.sparkContext.jobProgressListener.failedJobs.size
  }
