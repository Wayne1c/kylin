...
public class SparkMergingDictionary extends AbstractApplication implements Serializable {
    ...
    public static final Option OPTION_INCLUDE_MERGING_STATS = OptionBuilder.withArgName("includeMergingStats").hasArg()
            .isRequired(true).withDescription("dump metadata include merging statistics file")
            .create("includeMergingStats");
    ...
    public SparkMergingDictionary() {
        ...
        options.addOption(OPTION_INCLUDE_MERGING_STATS);
    }
    ...
    protected void execute(OptionsHelper optionsHelper) throws Exception {
        ...
        JavaPairRDD<Text, Text> colToDictPathRDD = indexRDD.mapToPair(new PairFunction<Integer, Text, Text>() {
            ...
            public Tuple2<Text, Text> call(Integer index) throws Exception {
                if (initialized == false) {
                    synchronized (SparkMergingDictionary.class) {
                        if (initialized == false) {
                            kylinConfig = AbstractHadoopJob.loadKylinConfigFromHdfs(sConf, metaUrl);
                ...
                if (index == columnLength) {
                    ...
                    for (CubeSegment cubeSegment : mergingSegments) {
                        String filePath = CubeSegment.getStatisticsResourcePath(cubeSegment.getCubeDesc().getName(), cubeSegment.getUuid());
