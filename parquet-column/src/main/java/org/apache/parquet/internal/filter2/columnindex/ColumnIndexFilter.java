...
public class ColumnIndexFilter implements Visitor<RowRanges> {
   ...
   */
  //= 返回匹配的页的开始row到结束row位置
  //= 再通过ColumnIndexStore获取这个column的ColumnIndex和OffsetIndex，
  //= 使用ColumnIndex列的最大值和最小值去和filter的条件相比较，找出符合条件的page（pageIndex）
  //= 再通过pageIndex从OffsetIndex里找到对应page的row start和row end，产生RowRange
  //= 如果有And，Or等条件，再把多个RowRanges进行合并或者交叉
