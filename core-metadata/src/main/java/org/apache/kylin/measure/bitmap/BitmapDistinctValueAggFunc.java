...
public class BitmapDistinctValueAggFunc implements ParamAsMeasureCount {
    @Override
    ...
    public static String result(BitmapAggregator agg) {
        BitmapCounter finalState = agg.getState();
        String result = "";
        if (finalState != null && finalState.getCount() > 0) {
            result = "[" + StringUtils.join(finalState.iterator(), ",") + "]";
        }
        return result;
    }
}
